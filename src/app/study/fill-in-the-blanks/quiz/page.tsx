...
import { Trophy, RefreshCw, ChevronLeft, Target, PenTool, CheckCircle2, XCircle, Info } from "lucide-react"
              ...
              return (
                <Card key={idx} className="border-none shadow-md overflow-hidden rounded-[1.5rem] bg-white">
                  <div className={cn("px-6 py-3 flex items-center justify-between", isCorrect ? "bg-green-50" : "bg-red-50")}>
                    <Badge variant={isCorrect ? "default" : "destructive"} className="rounded-full">
                  ...
                  <CardContent className="p-6 space-y-3">
                    ...
                    <div className="grid gap-2 text-sm">
                      <div className={cn("p-3 rounded-lg flex items-center gap-2", isCorrect ? "bg-green-100/30" : "bg-red-100/30")}>
                        <span className="font-bold">Your choice:</span> {q.options[userAns]}
                      </div>
                      {!isCorrect && (
                        <div className="p-3 rounded-lg bg-green-100/30 border border-green-200">
                          <span className="font-bold">Correct choice:</span> {q.options[q.correct]}
                        </div>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground pt-2 border-t">
                      <strong className="text-foreground">Logic:</strong> {q.explanation}
                    </p>
                  </CardContent>
                </Card>
              )
