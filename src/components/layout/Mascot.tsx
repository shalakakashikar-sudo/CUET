...
export function Mascot() {
  ...
  return (
    <div className={cn(
      "fixed z-[60] flex flex-col items-end pointer-events-none transition-all duration-500",
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className={cn(
              "mb-4 bg-white rounded-[2rem] shadow-2xl border-4 border-primary/20 font-black text-primary relative pointer-events-auto text-center parlour-stripes transition-all duration-300",
              isMobile ? "p-3 max-w-[160px] text-[10px]" : "p-5 max-w-[240px] text-sm"
            )}
          >
            {message}
            <div className={cn(
              "absolute bg-white border-r-4 border-b-4 border-primary/20 rotate-45",
              isMobile ? "-bottom-1.5 right-6 w-3 h-3" : "-bottom-2 right-10 w-4 h-4"
            )} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        >
          ...
          <g clipPath="url(#biteClip)">
            ...
            <motion.g animate={{ x: eyeOffset.x, y: eyeOffset.y - 12 }}>
              <motion.circle 
                cx="24" cy="78" r="6" 
                fill="#FF99AA" 
                animate={{ scale: [1, 1.4, 1], opacity: 0.4 }} 
          ...
          <motion.g
            animate={{ x: showHands ? [-12, -15] : [0, 0], opacity: showHands ? 1 : 0 }}
